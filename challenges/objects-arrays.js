...
graduates.forEach(value => {
    return (contactInfo.push(`${value.first_name} ${value.email}`))
})

console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. 
This will be an array of objects. Log the result. */
const unisWithUni = [];
graduates.filter(value => {
    if (value === "Uni") {
        return unisWithUni.push(value.university).length
    }
})
console.log(unisWithUni) // -> 4
...
zooAnimals.forEach(value => {
    return (displayNames.push(`${value.animal_name} , ${value.scientific_name}`))
})
...
zooAnimals.map(lowCase => {
    return lowCaseAnimalNames.push(lowCase.animal_name.toLowerCase())
})
...
zooAnimals.filter(value => {
    if (value.population < 5) {
        return lowPopulationAnimals.push(value.animal_name)
    }
})
